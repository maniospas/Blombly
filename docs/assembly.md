...
| derive res var    | Runs the block of code referenced by the variable `var` in a copy of the current memory and stores any returned value in `res` (like inline that returns to the previous memory state). |
