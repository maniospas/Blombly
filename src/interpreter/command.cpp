...
Command::Command(const std::string& command, SourceFile* source_, int line_, CommandContext* descriptor_) 
    ...
    bbassert(argNames.size()<=5, "Internal error: Command with more than 3 arguments");
    if(nargs) result = variableManager.getId(argNames[1]);
    if(nargs>1) arg0 = variableManager.getId(argNames[2]);
    if(nargs>2) arg1 = variableManager.getId(argNames[3]);
    if(nargs>3) arg2 = variableManager.getId(argNames[4]);

    bbassert(nargs== 0 || result != variableManager.thisId || argNames[0] == "set" || argNames[0] == "setfinal" || argNames[0] == "get" || argNames[0] == "return",
...
std::string Command::toString() const {
    ...
    if(nargs) ret += variableManager.getSymbol(result);
    if(nargs>1) ret += " "+variableManager.getSymbol(arg0);
    if(nargs>2) ret += " "+variableManager.getSymbol(arg1);
    if(nargs>3) ret += " "+variableManager.getSymbol(arg2);
    return ret;
}
...
std::string Command::tocpp(bool first_assignment) const {
    ...
    if(operation==BUILTIN) {
        ...
        if(value->getType()==BB_BOOL) return (first_assignment?"bool ":"")+variableManager.getSymbol(args[0])+" = "+value->toString(nullptr)+";";
    ...
    ret += ");";
    return ret;*/
    bberror("Currently disableed conversion of commands to cpp");
}