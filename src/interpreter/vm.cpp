...
void preliminarySimpleChecks(std::vector<Command>* program) {
    ...
    for (const auto& command : *program) {
        ...
        if(size==0 && op!=END && op!=BEGINCACHE) bberrorexplain("Invalid bbvm code: "+command.toString(), "Expecting a return value (even if that is `#`)", getStackFrame(command));

        if(op==NOT) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`not` accepts exactly one argument after the return value", getStackFrame(command));
        if(op==AND) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`and` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==OR) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`or` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==EQ) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`eq` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==NEQ) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`neq` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==LE) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`le` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==GE) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`ge` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==LT) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`lt` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==GT) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`gt` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==ADD) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`add` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==SUB) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`sub` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==MUL) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`mul` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==MMUL) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`mul` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==MUL) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`mul` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==MMUL) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`mmul` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==DIV) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`div` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==LEN) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`len` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==POW) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`pow` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==LOG) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`log` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==PUSH) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`push` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==POP) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`pop` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==NEXT) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`next` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==PUT) bbassertexplain(size==4, "Invalid bbvm code: "+command.toString(), "`put` accepts exactly 3 arguments after the return value", getStackFrame(command));
        if(op==AT) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`at` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==SHAPE) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`shape` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==TOVECTOR) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`vector` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==TOLIST) bbassertexplain(size==2 || size==1, "Invalid bbvm code: "+command.toString(), "`list` accepts 1 or no arguments after the return value", getStackFrame(command));
        if(op==TOMAP) bbassertexplain(size==2 || size==1, "Invalid bbvm code: "+command.toString(), "`map` accepts 1 or no arguments after the return value", getStackFrame(command));
        if(op==TOBB_INT) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`int` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==TOBB_FLOAT) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`float` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==TOSTR) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`str` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==TOBB_BOOL) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`bool` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==TOFILE) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`file` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==SUM) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`sum` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==MAX) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`max` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==MIN) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`min` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==BUILTIN) bbassertexplain(size==1, "Invalid bbvm code: "+command.toString(), "`BUILTIN` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==BEGIN) bbassertexplain(size==1, "Invalid bbvm code: "+command.toString(), "`BEGIN` accepts no argument after the return value", getStackFrame(command));
        if(op==BEGINFINAL) bbassertexplain(size==1, "Invalid bbvm code: "+command.toString(), "`BEGINFINAL` accepts no argument after the return value", getStackFrame(command));
        if(op==BEGINCACHE) bbassertexplain(size==0, "Invalid bbvm code: "+command.toString(), "`CACHE` accepts no argument and, by exception, assigns to no value", getStackFrame(command));
        if(op==END) bbassertexplain(size==0, "Invalid bbvm code: "+command.toString(), "`END` accepts no argument and, by exception, assigns to no return value", getStackFrame(command));
        if(op==RETURN) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`return` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==FINAL) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`final` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==IS) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`IS` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==CALL) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`call` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==WHILE) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`while` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==IF) bbassertexplain(size==4 || size==3, "Invalid bbvm code: "+command.toString(), "`if` accepts 2 or 3 arguments after the return value", getStackFrame(command));
        if(op==NEW) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`new` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==BB_PRINT) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`print` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==INLINE) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`inline` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==GET) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`get` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==SET) bbassertexplain(size==4, "Invalid bbvm code: "+command.toString(), "`set` accepts exactly 3 arguments after the return value", getStackFrame(command));
        if(op==SETFINAL) bbassertexplain(size==4, "Invalid bbvm code: "+command.toString(), "`setfinal` accepts exactly 3 arguments after the return value", getStackFrame(command));
        if(op==DEFAULT) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`default` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==TIME) bbassertexplain(size==1, "Invalid bbvm code: "+command.toString(), "`default` accepts no arguments after the return value", getStackFrame(command));
        if(op==TOITER) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`iter` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==TRY) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`try` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==CATCH) bbassertexplain(size==4 || size==3, "Invalid bbvm code: "+command.toString(), "`catch` accepts 2 or 3 arguments after the return value", getStackFrame(command));
        if(op==FAIL) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`fail` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==EXISTS) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`exists` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==READ) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`read` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==CREATESERVER) bbassertexplain(size==3, "Invalid bbvm code: "+command.toString(), "`server` accepts exactly 2 arguments after the return value", getStackFrame(command));
        if(op==AS) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`as` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==TORANGE) bbassertexplain(size>=2 && size<=4, "Invalid bbvm code: "+command.toString(), "`range` accepts 1 to 3 arguments after the return value", getStackFrame(command));
        if(op==DEFER) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`defer` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==CLEAR) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`clear` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==MOVE) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`move` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==ISCACHED) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`ISCACHED` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==TOSQLITE) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`sqllite` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==TOGRAPHICS) bbassertexplain(size==4, "Invalid bbvm code: "+command.toString(), "`graphics` accepts exactly 3 arguments after the return value", getStackFrame(command));
        if(op==RANDOM) bbassertexplain(size==4, "Invalid bbvm code: "+command.toString(), "`random` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==RANDVECTOR) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`vector::consume` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==ZEROVECTOR) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`vector::zero` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==ALLOCVECTOR) bbassertexplain(size==2, "Invalid bbvm code: "+command.toString(), "`vector::alloc` accepts exactly 1 argument after the return value", getStackFrame(command));
        if(op==LISTELEMENT) bbassertexplain(size>=2, "Invalid bbvm code: "+command.toString(), "`list::element` accepts at least 1 argument after the return value", getStackFrame(command));
        if(op==LISTGATHER) bbassertexplain(size>=2, "Invalid bbvm code: "+command.toString(), "`list::gather` accepts at least 1 argument after the return value", getStackFrame(command));
    }
